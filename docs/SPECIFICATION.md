...
1. `PlannerWidgetSync.syncFromPlannerDatabase` checks whether a session exists.
...
The method name `syncFromPlannerDatabase` is historical. In the current implementation, it syncs from the planner repository/API path, not from Room.
...
- Widget update/resizing triggers `PlannerWidgetSync.syncFromPlannerDatabase` and immediately renders from the latest available snapshot.
